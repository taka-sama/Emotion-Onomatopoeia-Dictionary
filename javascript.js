...
class EmotionObject{
  ...
  getOnomatopoeiaWords() {
    ...
    for (let i = 0; i < this.onomatopoeia.length; i++) {
      let currOnomato = this.onomatopoeia[i];
      let word = new Word(currOnomato, dictionary[currOnomato], pictureDictionary[currOnomato]);
      onomatopoeiaWords.push(word);
    }
  ...
  getHtmlContainerString(wordArr) {
    let containerOnomaSec =
      `
      <div id="${this.emotion}" class="text-left px-3 pb-4 d-md-flex flex-md-wrap justify-content-md-center" style="background-color: ${this.color}">
        <div class="col-12 py-4 px-2" style="color: white;">
          <h2>${this.emotion}</h2>
    ...
    for (let i = 0; i < wordArr.length; i++) {
      containerOnomaSec +=
        `
        <div class="d-flex col-md-5 mt-3 mx-md-3 col-lg-3 py-2 bg-white">
          <div class="col-8">
            <h4>${wordArr[i].word}</h4>
            <p>${wordArr[i].defintion}</p>
          </div>
          <div class="col-4">
            <img src="${wordArr[i].pictureUrl}">
          </div>
        </div>
        `
    }
    containerOnomaSec += "</div>";
    return containerOnomaSec;
  }
}
...
let categoryContainer = "<div class='container d-flex flex-wrap justify-content-center'>";
for (let i = 0; i < emotions.length; i++){
  let currFace = HelperFunction.faceSelect(emotions[i]["emotion"]);

  categoryContainer +=
    `
      <div class="d-flex justify-content-center col-md-4 col-lg-3 text-center py-3 m-4" style="background-color: ${emotions[i]["color"]}; color: white;">
        <a href="#${emotions[i]["emotion"]}">
          <h4>${emotions[i]["emotion"]}</h4>
          <p class="face">${currFace}</p>
...
//各emotionのオノマトペを作る
for (let i = 0; i < emotions.length; i++) {
  let currOnomatoWord = emotions[i].getHtmlContainerString(emotions[i].getOnomatopoeiaWords(emotions[i]["onomatopoeia"]));
  containerSec += currOnomatoWord;
};

let mainTargetDiv = document.getElementById("target");
...
mainTargetDiv.innerHTML = htmlString;