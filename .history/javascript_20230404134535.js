...
class EmotionObject{
  ...
  getHtmlContainerString() {
    var containerSection = "<div>";
    const keys = emotions.keys(this);
    let currSection = emotions[i]
    for (let i = 0; i < keys.length; i++) {
      containerSection +=
      `
        <div id="sec${i}" class="bg-${currSection.color} big-square">
          <h2>${currSection.emotion}</h2>
          <p>${currSection.description}</p>
          <div class="onomatopoeias">
            <div class="left-item">
              <p>${currSection.onomatopoeia}</p>
              <p>${dictionary.currSection.onomatopoeia}</p>
            </div>
            <div class="right-item">
              <img src="${pictureDictionary.currSection.onomatopoeia}">
            </div>
          </div>
        </div>
      `
...
document.getElementById("target").innerHTML = containerSection;
...
const myEmotionObject = new EmotionObject("angry", "feeling or showing strong annoyance, displeasure, or hostility; full of anger.", "red", ["bark", "grunt", "roar", "whack", "smack", "hiss"]);
console.log(myEmotionObject.getOnomatopoeiaWords());